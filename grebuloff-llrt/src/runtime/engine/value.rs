...
impl Value {
    ...
    pub fn into<T: FromValue>(self, engine: &JsEngine) -> Result<T> {
    ...
    pub fn coerce_number(&self, engine: &JsEngine) -> Result<f64> {
        match self {
            &Value::Number(n) => Ok(n),
    ...
    pub fn coerce_string(&self, engine: &JsEngine) -> Result<String> {
        match self {
            &Value::String(ref s) => Ok(s.clone()),
            value => engine.try_catch(|scope| {
                let maybe = value.to_v8_value(scope).to_string(scope);
                engine.exception(scope).map(|_| String {
                    engine: engine.clone(),
...
impl Values {
    ...
    pub fn from<T: FromValue>(&self, engine: &JsEngine, index: usize) -> Result<T> {
        T::from_value(
            self.0
                .get(index)
                .map(Clone::clone)
    ...
    pub fn into<T: FromValues>(self, engine: &JsEngine) -> Result<T> {
